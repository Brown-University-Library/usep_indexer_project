...
def remove_index_entry(filename: str | None = None, inscription_id: str | None = None) -> None:
    ...
    Called by: run_remove_entry(), run_remove_entry_via_id()
    """
...
def run_update_index(files_updated: list[str], files_removed: list[str]) -> None:
    """
    Enqueues per-inscription incremental Solr jobs.

    Called by: processor.run_xinclude_updater()
    """
...
def run_update_entry(updated_file_path: str) -> None:
    """
    Updates one Solr entry from a queued source path.

    Called by: run_update_index(), reindex.run_enqueue_all_index_updates()
    """
    update_index_entry(Path(updated_file_path).name)
    return


def run_remove_entry(removed_file_path: str) -> None:
    """
    Removes one Solr entry from a queued source path.
...
def run_remove_entry_via_id(id_to_remove: str) -> None:
    """
    Removes one Solr entry from a queued explicit ID.

    Called by: reindex.run_enqueue_all_index_updates()
    """
